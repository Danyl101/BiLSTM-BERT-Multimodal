...
def train_model():
    model=BiLSTMModel(input_size=lstm_params['input_size'], hidden_size=lstm_params['hidden_size']
                     ,num_layers=lstm_params['num_layers'],dropout=lstm_params['dropout'],
                     batch_size=lstm_params['batch_size']).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lstm_params['learning_rate']) #Defines optimizer
