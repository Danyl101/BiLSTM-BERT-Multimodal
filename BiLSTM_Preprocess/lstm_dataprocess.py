...
def preprocess():
    #Importing raw data
    data=pd.read_csv(config['paths']['lstm']['nifty_raw_data'])
    print(data.head())
    ...
    finaltrain.to_csv(config['paths']['lstm']['nifty_train_scaled_data'], index=False)
    finalval.to_csv(config['paths']['lstm']['nifty_validation_scaled_data'], index=False)
    finaltest.to_csv(config['paths']['lstm']['nifty_test_scaled_data'], index=False)
